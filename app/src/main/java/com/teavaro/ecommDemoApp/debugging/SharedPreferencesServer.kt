...
class SharedPreferencesServer(context: Context, prefsFileName: String) : NanoHTTPD(8080) {
    ...
    private fun updateValue(session: IHTTPSession): Response {
        try {
            ...
            if (key != null && value != null) {
                ...
                return newFixedLengthResponse(Response.Status.OK, "text/plain", "Updated")
            } else {
                // If key is not found, return a Bad Request response
                println("❌ Key or value not found in request!")
                return newFixedLengthResponse(Response.Status.BAD_REQUEST, "text/plain", "Invalid key or value")
            }
        } catch (e: Exception) {
            // Handle errors that might occur during parsing
            println("⚠️ Error parsing the request body: ${e.message}")
            return newFixedLengthResponse(Response.Status.BAD_REQUEST, "text/plain", "Error updating value: ${e.message}")
        }
    }
    ...
    private fun deleteValue(session: IHTTPSession): Response {
        try {
            ...
            if (key != null) {
                ...
                return newFixedLengthResponse(Response.Status.BAD_REQUEST, "text/plain", "Key not found")
            }
        } catch (e: Exception) {
    ...
    companion object {
        ...
        fun start(context: Context, prefsFileName: String) {
            ...
            try {
                this.server.start()
                Timber.tag("SharedPreferencesServer").d("Server started at http://localhost:8080")
            } catch (e: IOException) {
                e.printStackTrace()
            }
        }
